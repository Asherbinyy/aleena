...
import 'package:aleena/src/features/DrawerFeature/bloc/repository/return_orders_repo.dart';
...
class ReturnOrdersController extends GetxController{
  ...
  ReturnOrdersRepository _returnOrdersRepository = ReturnOrdersRepository();
  Future<void> fetchReturnOrders()async{
  status = RequestStatus.loading;
    var response = await _returnOrdersRepository.fetchReturnOrders();
  if (response.statusCode == 200 && response.data["status"] == true) {
      print("request operation success");
      lastOrder.clear();
      for (var item in response.data['cancelOrder']) {
